...
def get_dvc_info(ip,community_string):
    ...
    vendor = ""
    dvc_info = snmpdvcinfo.get_snmp(ip, [OID_GENERAL_DEVICE_INFO], hlapi.CommunityData(community_string))
    ...
    if dvc_info != 0 and dvc_info[OID_GENERAL_DEVICE_INFO] != "": # --> check for snmp error --> every device should have a sys description string
        ...
        dvc_info = snmpdvcinfo.get_snmp(ip, [OID_GENERAL_DEVICE_INFO_2], hlapi.CommunityData(community_string))
